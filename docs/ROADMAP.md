...
| `Schoenflies.HasMeshSteps` | `StageRecursion.lean` | the interior homeomorphism | **done, unconditional** — `Schoenflies.hasMeshSteps'` (`MeshStepsClosed.lean`) needs only `S₀.CombInvariants` and `IsSeparating C`. Every link of the chain is a theorem: `hasMeshSteps` (`MeshSteps.lean`, the quantitative half and the fresh-anchor supply), `hasMeshTransfers` (`MeshTransfer.lean`, via the repaired `finite_transfer_back'` and `Graph.renameEdges`), `hasMeshOverlays_of` (`MeshOverlay.lean`, `meshOverlayGraph` with `bad = ∅`), `hasTwoConnectedMeshOverlays_of` (`MeshTwoConnected.lean`, the ear engine `of_spanning_paths` needing no fresh interiors), `hasTwoConnectedMeshCores` (`MeshCore.lean`, whose new graph theory is the descent converse `isTwoConnected_descend`), and `hasTwoConnectedSkelCores` (`SkelCore.lean`, whose `Graph.IsTwoConnected.of_edge_paths` transports 2-connectivity across edge types directly, sidestepping `IsSubdivisionOf`'s single-type limit). Historical detail stays in the module docstrings |
