...
| `thm:finite-transfer` (b) | partial | `FiniteTransferTarget.lean` constructs the target common subdivision, fully implements the target-path relabelling and matched reverse split, and carries out the relative-ear induction. It derives off-curve accessibility, constructs the compact nonboundary-edge carrier, and applies `FreshAccess.lean` to discharge the fresh-anchor geometry. `finite_transfer_toward_source_of_freshInvariant` now leaves only `TargetEarFreshInvariant`: the prescribed outer-cycle ear order must identify each wild-boundary endpoint as a strongly accessible anchor absent from the current nonboundary carrier and incident with one unique source face |
