...
| `prop:local-grid-attachment` | conditional (crosscut-ear assembly) | `LocalGrid.lean` (`localGrid`, the diameter clause) + `GridAttach.lean` (the crosscut factory and the general component-joining loop) + `SourceOverlay.lean` + `SourceAttachment.lean`. The raw overlay now proves every source-extension field from two common source/grid vertices by gluing plane-subdivision traces; at an admissible stage one common point also gives connectedness off the outer curve. For windows with fewer than two intersections, `SourceAttachment.crosscutOverlay` implements the missing inner object: its exact carrier is the old compact core, one auxiliary face crosscut, and the grid; it preserves all required vertices and proves planarity, domain containment, edge geometry, old-edge absorption, and the grid subdivision certificate. Remaining: adjoin the wild outer graph to that crosscut overlay and prove the crosscut trace is an ear on the old 2-connected trace. |
