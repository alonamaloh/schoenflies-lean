...
| `prop:local-grid-attachment` | conditional (boundary-touching crosscut case) | `LocalGrid.lean` (`localGrid`, the diameter clause) + `GridAttach.lean` (the crosscut factory and the general component-joining loop) + `SourceOverlay.lean` + `SourceAttachment.lean`. The raw overlay proves every source-extension field from two common source/grid vertices. For the degenerate cases, `SourceAttachment.crosscutOverlay` now adjoins the wild outer graph, traces the subdivided crosscut as an exact path, attaches it as an ear to the old 2-connected source trace, glues the grid trace along a swallowed grid edge, proves connectedness off the outer curve, and packages the result as a source extension. The remaining geometric case is when a face crosscut ends in the interior of a wild outer edge: those endpoint cuts must also subdivide the outer graph (or one must prove the selected face crosscut avoids the outer curve). |
