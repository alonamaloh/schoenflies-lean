...
| `prop:local-grid-attachment` | conditional (final component-joining ear) | `LocalGrid.lean` (`localGrid`, the diameter clause) + `GridAttach.lean` (the crosscut factory and the general component-joining loop) + `SourceOverlay.lean` + `SourceAttachment.lean` + `OverlayExtension.lean`. The raw overlay proves every source-extension field from two common source/grid vertices. `SourceAttachment` exports the face-crosscut data, swallows a selected grid edge, traces the subdivided crosscut as an exact ear, and packages a complete source extension whenever the face frontier avoids the wild outer curve. In the boundary-touching branch it performs matched subdivisions at both crosscut endpoints, yielding a plane, 2-connected, domain-contained mixed overlay; it is already a complete source extension if either endpoint is nonouter. `OverlayExtension` re-overlays any current straight graph together with new joining segments and proves an exact plane-subdivision certificate, providing the finite engine for the last ear. Remaining: use that engine to adjoin a polygonal joining ear when both crosscut endpoints are outer, so deleting the wild curve becomes connected. |
