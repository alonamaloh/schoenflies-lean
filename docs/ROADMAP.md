...
| `Schoenflies.HasGridExtensions` | `GridSteps.lean` | `HasGridSteps` | **partially assembled** in `GridExtension.lean` (merged from an intentionally stopped run — resume there, not from scratch): `gridExtGraph` is the union of the stage's outer part with `gridAttachGraph` over `skeletonSegs ++ joins ++ grid`, `gridExtGraph_isDrawing` is proved, and `isSourceExtensionOver_gridExtGraph` assembles every transfer hypothesis from placement facts plus two designed inputs — `htc` (the union's 2-connectivity, the `hΓ` assembly, hypothesis `HasGridUnionTwoConnected`) and `hconn` (the open part connected, from `GridComponents.lean`'s joining loop). **Missing**: the stage-level instantiation (choose `gsegs` via `skeletonSegs`, discharge the placement facts and `hchain`, apply `exists_reps_hcov` for the joins, transport along `nonempty_gridExtensionData_of_over`) and the `grid_subset` clause |
