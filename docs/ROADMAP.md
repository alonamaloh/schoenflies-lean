...
| `prop:local-grid-attachment` | conditional (final component-joining ear) | `LocalGrid.lean` (`localGrid`, the diameter clause) + `GridAttach.lean` (the crosscut factory and the general component-joining loop) + `SourceOverlay.lean` + `SourceAttachment.lean`. The raw overlay proves every source-extension field from two common source/grid vertices. `SourceAttachment` now exports the face-crosscut data, swallows a selected grid edge, traces the subdivided crosscut as an exact ear, and packages a complete source extension whenever the selected face frontier avoids the wild outer curve. In the boundary-touching branch it performs a matched preliminary subdivision at both crosscut endpoints; this makes endpoints inside wild outer edges genuine outer vertices and yields a plane, 2-connected, domain-contained mixed overlay containing the refined skeleton and full grid. Remaining: adjoin the manuscript's polygonal component-joining ear so that deleting the wild outer curve is connected when both crosscut endpoints lie on it. |
