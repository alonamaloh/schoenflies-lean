...
| `Schoenflies.HasGridUnionTwoConnected` | `GridExtension.lean` | `HasGridExtensions` | 2-connectivity of the assembled union — the `hΓ` assembly, the direction-(a) twin of `HasTwoConnectedMeshOverlays`. Existential over the auxiliary segments `xsegs`/`xtra` (the degenerate-case crosscut) then universal over representatives and simple join families; the fixed-graph form without `xsegs` would be **false** in the degenerate cases. Discharger ingredients all on `main`, listed in its docstring; the real content is the ear induction along subdivided joining arcs |
