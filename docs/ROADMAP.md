...
|---|---|---|---|
| `Graph.Descent` | `OuterChain.lean` | `lem:outer-chain`, hence `thm:arc-complement`, hence `thm:jordan` | the blueprint's crosscut paragraph (tex 916–946). Split into `Graph.CrosscutExists` (combinatorial) and `Graph.CrosscutEncloses` (geometric) by `Graph.IsPlaneChain.descent_of_crosscut`; discharge those two, not this |
| `Schoenflies.HasArcCollars` | `CrosscutAtMostTwo.lean` | `lem:crosscut-at-most-two`, hence `thm:general-crosscut` | Lemma 1.8 (b) for an **arc**. The whole strip apparatus exists for a `ClosedPolygon` only (`Strip.lean`, `StripConstants.lean`, `StripConnected.lean`, `StripLocal.lean`) |
...
| `lem:polygonal-collar` (b) | **open** | for an arc: see `HasArcCollars` above |
...
| `lem:outer-chain` (H9) | conditional (`Graph.Descent`) | `OuterChain.lean` |
...
| `thm:arc-complement` | in progress | scaffolding in `ArcComplementPrep.lean` |
| `lem:accessible-dense` | in progress | |
| `thm:jordan` | in progress | |
| `lem:crosscut-at-most-two` | conditional (`HasArcCollars`) | `CrosscutAtMostTwo.lean` |
| `thm:general-crosscut` (H10) | in progress | |
...
| `lem:local-skeleton-structure` | partial | `SkeletonLocal.lean` — radial segments done, sector decomposition and the distinct-directions derivation open |
| `prop:anchored-square-mesh` | partial | `SquareMesh.lean` — geometry and diameter bounds done, 2-connectivity and the outer cycle open |
