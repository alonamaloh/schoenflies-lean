...
| `thm:finite-transfer` (b) | partial | `FiniteTransferTarget.lean` constructs the target common subdivision, fully implements the target-path relabelling and matched reverse split, and carries out the relative-ear induction. The evolving skeleton map now provably extends the original map. `TargetBoundaryAnchored` turns an ambient nonouter boundary edge into strong accessibility of its original source preimage, and `FiniteTransferTargetMesh.lean` discharges that condition from anchored-square-mesh clause 4. `finite_transfer_toward_source_squareMesh` now leaves only `TargetEarFreshCombinatorics`: the prescribed ear order must prove that each wild-boundary endpoint is absent from the current nonboundary carrier and incident with one unique source face. `source_pos_notMem_nonboundaryGraph_of_outerOnlyAt` discharges the first whenever the current abstract vertex has only outer incident edges |
