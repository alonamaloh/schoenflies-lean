...
| `Schoenflies.HasTwoConnectedMeshCores`, `HasTwoConnectedSkelCores` | `MeshTwoConnected.lean` | `HasTwoConnectedCores` → the mesh chain | the two subdivision bridges: a 2-connected subgraph of the overlay spanning the mesh-side (resp. skeleton-side) vertices. Mesh side: same-edge-type refinement of `squareMesh`, extra cut points one at a time, seeded by `meshGraph_isTwoConnected`. Skeleton side: the cross-type subdivision of `Γ'` — `IsSubdivisionOf` cannot be stated across the `γ`-vs-`Piece` gap, which is why the hypothesis is cut at "spanning 2-connected core" rather than at a subdivision relation; `Graph.renameEdges` may enable a subdivision-with-relabelling analogue |
