...
```sh
/tmp/claude-1000/-home-alvaro-claude-schoenflies/cd0a4752-0d99-41c7-a3ad-b4ee811d9c2d/scratchpad/new-worker.sh <yourname>
```
...
Build **only your own module**, by name:

```sh
cd <worktree> && export PATH="$HOME/.elan/bin:$PATH" && lake build Schoenflies.<YourModule>
```
...
Warm builds are ~2s. If a build takes minutes, something is wrong — say so rather than waiting.
...
Sixteen modules, 3802 lines, all `sorry`-free. **Read the ones your module touches before
