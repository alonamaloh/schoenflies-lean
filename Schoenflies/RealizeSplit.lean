...
`Graph.walkPointSet` and the theorems this module runs on — `Graph.IsPath.map`,
`Graph.IsPathGraph.map`, and above all `Graph.IsDrawing.isArcBetween_walkPointSet`, **the point
set of a drawn path is a simple arc between its two ends** — were written here for want of a
home and are now in `Schoenflies/Graph/DrawnWalk.lean`, where the ear steps of both directions
of `thm:finite-transfer` reach them without importing any of the cell-structure layer.
-/
...
/-- The drawn skeleton is covered by the open cells of dimension 0 and 1. The dichotomy is
`Realization.exists_cell_of_mem_skeletonSet`, in `Schoenflies/CombinatorialInvariance.lean`;
this is it in the `cellUnion` form, which is what a decomposition argument consumes. -/
theorem skeletonSet_subset_cellUnion :
    R.skeletonSet ⊆ R.cellUnion (V(S.skel) ∪ E(S.skel)) := fun _ hx =>
  let ⟨_, hκ, hxκ⟩ := exists_cell_of_mem_skeletonSet hx
  cell_subset_cellUnion hκ hxκ
