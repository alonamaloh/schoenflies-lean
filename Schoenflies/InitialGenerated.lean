...
* `Schoenflies.initialBoundaryWalks` — the boundary-walk invariant at stage 0, the `walks`
  field of `Schoenflies.GeneratedPair`, with `Schoenflies.isCycle_initBoundary` (the two
  boundary lists are cycles) and `Schoenflies.pathCells_initBoundary` (their cells are the
  `faceCells`) as its two halves.
...
/-! ### The boundary-walk invariant at stage 0

`InitialPairFixed.lean` proved the two halves separately — `isWalk_initBoundary_false`,
`.._true` that the two boundary lists are closed walks, and `mem_faceCells_iff` that the cells
they run through are exactly the `faceCells` that `initSub` declares below each 2-cell. What
was left was the vertex-simplicity, and the packaging: `GeneratedPair` carries a
`CellStructure.BoundaryWalks`, so stage 0 has to supply one. -/
...
theorem pathCells_initBoundary (k : Bool) :
  ...
  ext c
  simp only [CellStructure.pathCells, Graph.walkVertices, Graph.coveredVertices, mem_union,
    mem_setOf_eq, mem_insert_iff, mem_faceCells_iff, initialStructure_skel]
  constructor
  · rintro (hc | rfl | hc)
...
theorem subcells_face_sdiff (k : Bool) :
  ...
  ext c
  rw [mem_sdiff, CellStructure.mem_subcells_iff, mem_singleton_iff, initialStructure_sub]
  constructor
  · rintro ⟨⟨-, h⟩, hne⟩
