...
| `Schoenflies.HasTwoConnectedMeshOverlays` | `MeshOverlay.lean` | `HasMeshOverlays` | 2-connectivity of the mesh overlay, existential over the joining arcs — **deliberately not** "`IsTwoConnected` of the constructed graph for the extracted joins", which is false (a chain doubling back subdivides to a pendant edge); the discharger chooses simple chains. Route in the docstring: skeleton restriction subdivides `Γ'`, mesh restriction subdivides `squareMesh`, glue with `IsTwoConnected.union`, joins as ears |
