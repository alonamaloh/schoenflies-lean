...
| `prop:local-grid-attachment` | conditional (two global attachment properties) | `LocalGrid.lean` (`localGrid`, the diameter clause) + `GridAttach.lean` (the overlay, the crosscut factory, the component-joining loop, and the construction as `def`s) + `SourceOverlay.lean`. The latter now extracts an exact finite segment cover of every generated pair's nonboundary source carrier, overlays one local grid, relabels its edges away from all old cell names, adjoins the original wild outer graph, and proves finiteness, planarity, exact carrier, old-vertex and old-skeleton inclusion, domain containment, edge dichotomy, and the full subdivision/absorption clause. Its `LocalOverlayRelabeling.isSourceExtension` leaves exactly 2-connectivity and connectedness off the outer curve; these are the `hΓ`/`hcov` content already isolated by `GridAttach.lean`. |
