...
1. the stage recursion — both directions of `thm:finite-transfer` are implemented.  A bare
   ...
   edge-dichotomy obligations.  Its 2-connectivity is now unconditional under the mesh's usual
...
| `prop:local-grid-attachment` | conditional (source/grid intersection case split) | `LocalGrid.lean` (`localGrid`, the diameter clause) + `GridAttach.lean` (the crosscut factory and the general component-joining loop) + `SourceOverlay.lean` + `SourceAttachment.lean` + `OverlayExtension.lean` + `SourceJoining.lean`. The raw overlay proves every source-extension field from two common source/grid vertices. `SourceAttachment` exports the face-crosscut data, swallows a selected grid edge, traces the subdivided crosscut as an exact ear, and performs matched subdivisions at both boundary-touching endpoints. `SourceJoining` closes the formerly missing branch: it re-overlays a simple polygonal joining arc, attaches its exact trace as a second ear, proves connectedness after deleting the wild outer curve, and packages the result as a source extension. In the actual Jordan domain the joining arc's open connected ambient-region hypotheses are automatic. It also selects the required bounded source face from any grid edge whose relative interior misses the source skeleton. Remaining: prove and assemble the finite dichotomy that either the source/grid overlay has two distinct common vertices or some raw grid edge has skeleton-disjoint relative interior, then expose the single unconditional stage-construction interface. |
| `lem:grid-star-estimate`, `prop:shrinking-stars`, `lem:anchor-density` | partial | `QuantitativeStages.lean` constructs an unconditional reverse successor below any prescribed positive scale, bounds every resulting target star, and proves that its stronger target-face mesh estimate survives the following forward transfer. The remaining quantitative-refinement obligation is the forward local-grid source extension and the pointwise source-star estimate. The metric half is ready: `Windows.lean` has `supRadius` (the ℓ^∞ distance to a compact set, with attainment, positivity and the 1-Lipschitz property), `windowRadius` / `window` / `openWindow` with the blueprint's three inequalities and `W_n(p) ⊆ D`, the arithmetic of `prop:shrinking-stars` (`mem_openWindow_of_supDist_lt`), and the two sequences (`recur`, `tendsto_two_pow_neg`) |
