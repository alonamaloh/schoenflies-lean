...
| `prop:local-grid-attachment` | conditional (two common source/grid vertices) | `LocalGrid.lean` (`localGrid`, the diameter clause) + `GridAttach.lean` (the overlay, the crosscut factory, and the general component-joining loop) + `SourceOverlay.lean`. The latter extracts an exact finite segment cover of every generated pair's nonboundary source carrier, overlays one local grid, freshly relabels its edges, adjoins the original wild outer graph, and proves every local source-extension field. Both the old source graph and the raw grid have plane-subdivision traces in the mixed graph; two distinct common overlay vertices therefore give 2-connectivity. At an admissible stage the old open skeleton is connected, and one common grid point makes the complement of the outer curve connected. Thus `LocalOverlayRelabeling.isSourceExtension_of_source_connected_two_common` leaves only the geometric production of those two common vertices (or the auxiliary-crosscut cases already prepared in `GridAttach.lean`). |
