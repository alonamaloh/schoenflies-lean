...
| `thm:finite-transfer` (b) | partial | `FiniteTransferTarget.lean` constructs the target common subdivision, fully implements the target-path relabelling and matched reverse split, and carries out the relative-ear induction. The evolving skeleton map now provably extends the original map. `TargetBoundaryAnchored` turns an ambient nonouter boundary edge into strong accessibility of its original source preimage, and `FiniteTransferTargetMesh.lean` discharges that condition from anchored-square-mesh clause 4. Local carrier reflection plus mesh clause 4 now prove every new wild-boundary endpoint is outer-only. A simple face boundary supplies two incident edges there, so `OuterIncidenceAtMostTwo` and `outerEdge_unique` force the selected face to be the unique incident face. Consequently `finite_transfer_toward_source_squareMesh_of_outerIncidenceAtMostTwo` reduces direction (b) to one static obligation: propagate the locally two-branched outer-cycle invariant through generated structures |
