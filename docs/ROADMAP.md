...
| `lem:grid-star-estimate`, `prop:shrinking-stars`, `lem:anchor-density` | partial (stage recursion only) | `QuantitativeStages.lean` constructs the unconditional reverse successor and its uniform target-star estimate. `QuantitativeForwardStages.lean` constructs the unconditional forward local-grid successor, proves `lem:grid-star-estimate` (`diam_sourceStar_le`), preserves the target face mesh, and packages the two half-steps as an unconditional `QuantitativeSuccessor` with one composed transition and both quantitative star bounds. `Windows.lean` supplies the window-catching arithmetic and the null/recurrent sequences. Remaining: recursively choose these successors along a recurrent dense sequence of window centres and prove the two resulting `Tendsto` fields required by `StageSequence`. |
