...
| `thm:finite-transfer` (b) | partial | `FiniteTransferTarget.lean` constructs the target common subdivision and carries out the relative-ear induction, reducing the theorem to `TargetEarStep`. Its geometric ingredient beyond (a), source accessibility at a fresh anchor on the wild curve, is closed in `FreshAccess.lean` (`polyAccessible_of_stronglyAccessible`); the remaining work is to maintain the prescribed outer-cycle / unique-incident-face invariant while constructing each reverse ear step |
