...
| `Schoenflies.HasLimitHomeomorphism` | `BoundaryContinuity2.lean` | `thm:square-extension` | **all four conjuncts are now supplied**: `AnchorDensity.lean`'s `anchoredStageSequence` (the recursion re-instantiated with the anchor data) delivers the exact ∃-body — dense anchors, `isHomeoOn_F`, `hasAnchorCrosscuts`, `hasSpokes` — from the two *anchored* choosers `HasAnchoredGridSteps` / `HasAnchoredMeshSteps`, which subsume the plain ones (`.hasGridSteps` / `.hasMeshSteps`). One caveat for the final assembler: the composition delivers the body for `u := d.u` (the boundary map `InitialData` manufactures), and `HasLimitHomeomorphism` quantifies over a *given* `u` — an initial-pair constructor for a prescribed `u` is missing, though `exists_initialData`'s proof visibly supports one (it uses `u` only through `IsSetHomeoOn`) |
...
| `Schoenflies.HasGridExtensions` | `GridSteps.lean` | `HasGridSteps` | **discharged** by `Schoenflies.hasGridExtensions_of` (`GridExtension.lean`) from `HasGridUnionTwoConnected` below — `gridExtGraph` (the stage's outer part ∪ `gridAttachGraph` over `skeletonSegs ++ xsegs ++ joins ++ grid`, over `γ ⊕ Piece`), its drawing, `meetsFinitely_skeletonSegs`, the connectivity via `gridJoin` + `exists_reps_hcov`, and every `IsSourceExtensionOver` clause are all proved; end-to-end `example` reaches `StageSequence` |
| `Schoenflies.HasGridUnionTwoConnected` | `GridExtension.lean` | `HasGridExtensions` | **discharged** by `hasGridUnionTwoConnected_of` (`GridTwoConnected.lean`) from `HasGridAnchoredCores` below — the grid half (`gridPartGraph` 2-connected for any subdivision, spanning every overlay vertex on the grid, glued at two anchors), plus the sum-type/renaming/orientation plumbing |
| `Schoenflies.HasGridAnchoredCores` | `GridTwoConnected.lean` | `HasGridUnionTwoConnected` | the large half, honestly: a 2-connected core off the grid spanning the drawn 0-cells, skeleton chains, `xsegs` and joins. Retains (a) the cross-type subdivision transport for outer cycle + skeleton chains — the direction-(a) twin of `HasTwoConnectedSkelCores`, so one solution likely serves both; (b) the `xsegs` window-crossing crosscut construction; (c) join absorption, blocked by an interface gap: `IsJoinFamily` makes the join's *carrier* simple but not its *chain* (`[a,b,a,b]` qualifies), and joins are universally quantified on this side — **a candidate repair is to strengthen `IsJoinFamily` to chain-level simplicity in `GridExtension.lean`**, whose own `gridJoin` constructs the joins and can supply it. Also recorded: `gridAttachGraph_isTwoConnected`'s `hΓ`/`hK` carve is unusable on the source side (the Γ-side excludes the outer cycle and is typically not 2-connected alone); the carve must happen at the `sumUnion` level, which `HasGridAnchoredCores` does |
...
| `Schoenflies.HasMeshOverlays` | `MeshTransfer.lean` | `HasMeshTransfers` | **discharged** by `Schoenflies.hasMeshOverlays_of` (`MeshOverlay.lean`) from `HasTwoConnectedMeshOverlays` below — `meshOverlayGraph` is the construction, nine of the ten `MeshOverlayExtension` fields are theorems, `bad = ∅`, and the joining arcs (`JoinsFor`, `exists_joinsFor`) carry the load-bearing connectivity |
| `Schoenflies.HasTwoConnectedMeshOverlays` | `MeshOverlay.lean` | `HasMeshOverlays` | **discharged** by `hasTwoConnectedMeshOverlays_of` (`MeshTwoConnected.lean`) from `HasTwoConnectedCores` — the ear engine (`Graph.IsTwoConnected.of_spanning_paths`, which needs **no fresh interiors**, so crossing chains need no splitting induction), the simple-joins choice, and the two-fresh-point glue are all proved |
| `Schoenflies.HasTwoConnectedMeshCores` | `MeshTwoConnected.lean` | `HasTwoConnectedCores` | **a theorem** — `Schoenflies.hasTwoConnectedMeshCores` (`MeshCore.lean`), unconditional. The docstring's forward induction was not the route: the fix is to take the mesh's anchors to be the overlay's own cut list, so `meshGraph` *is* the desired graph further refined, then **descend** — the genuinely new graph theory is the converse of `IsSubdivisionOf.isTwoConnected` (`isTwoConnected_descend`, with the honest side conditions `x ≠ y` and `HasThreeVertices`). Several hoisting candidates flagged in the module |
| `Schoenflies.HasTwoConnectedSkelCores` | `MeshTwoConnected.lean` | `HasTwoConnectedCores` → the mesh chain | the skeleton-side bridge: a 2-connected subgraph of the overlay spanning the skeleton-side vertices. The cross-type subdivision of `Γ'` — `IsSubdivisionOf` cannot be stated across the `γ`-vs-`Piece` gap; `Graph.renameEdges` may bridge it, and `MeshCore.lean`'s `overlayGraph_isTwoConnected_of_extend` (cut-list invariance) may serve here too |
| anchor incidence | `SkeletonCrosscuts.lean` | `HasAnchorCrosscuts`, `HasSpokes` | **retired**: `AnchorDensity.lean`'s `IsAnchorVertexAt` invariant delivers exactly the hypothesis pair of `exists_anchor_crosscut_stage` (machine-checked by an `example`), carried through the anchored recursion. What survives of it is the `fresh_anchorAt` delta of `HasAnchoredMeshSteps` — a fresh point becomes a drawn 0-cell with a surviving spoke — genuinely a hypothesis today, because `IsTransferOfTgt`/`MeshOverlayExtension` never state `z ∈ V(H)` for fresh `z` (interface gap 4 of the AnchorDensity report). `Realization.HasPolygonalArcs` was already a theorem (`PolygonalSkeletonArcs.lean`) |
