...
| `thm:finite-transfer` (b) | partial | `FiniteTransferTarget.lean` constructs the target common subdivision, fully implements the target-path relabelling and matched reverse split, and carries out the relative-ear induction. The evolving skeleton map now provably extends the original map. `TargetBoundaryAnchored` turns an ambient nonouter boundary edge into strong accessibility of its original source preimage, and `FiniteTransferTargetMesh.lean` discharges that condition from anchored-square-mesh clause 4. Local carrier reflection plus mesh clause 4 prove every new wild-boundary endpoint is outer-only. A simple face boundary supplies two incident edges there, so `OuterIncidenceAtMostTwo` and `outerEdge_unique` force the selected face to be unique. `OuterEdgesFormCycle` implies the local bound and is now proved invariant under both generated-structure constructors; `InitialOuterCycle.lean` verifies it for the initial hexagon. Thus `finite_transfer_toward_source_squareMesh_of_outerCycle` closes the reverse-ear geometry and combinatorics. The remaining integration is to put the concrete square mesh and the `InitialCell`-named generated pair over one common infinite cell-name type (via the existing edge-relabel machinery), then use this theorem in the stage recursion |
